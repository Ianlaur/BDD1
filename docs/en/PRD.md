...
**Related Documents:** [Functional Specification](./functionalSpecification.md)
...
*See [Functional Specification - User Flows](./functionalSpecification.md#3-user-flows) for detailed solutions to these problems.*
...
*See [Section 9 - Success Criteria](#9-success-criteria) for detailed launch and growth metrics.*

---

## 2. User Personas

*These personas map to specific roles and permissions detailed in [Functional Specification - Actors and Roles](./functionalSpecification.md#2-actors-and-roles).*
...
*For detailed authentication flows and validation rules, see [Functional Specification - Authentication Flows](./functionalSpecification.md#31-authentication-flows).*
...
*See [Functional Specification - Association Discovery](./functionalSpecification.md#33-association-discovery-and-joining) for detailed search algorithms and filter logic.*
...
*See [Functional Specification - Membership Management](./functionalSpecification.md#36-membership-management) for state transitions and approval workflows.*
...
*See [Functional Specification - Event Management](./functionalSpecification.md#34-event-management) for detailed creation, registration, and cancellation flows. Also see [Section 6.3 - Event Creation Flow](#63-event-creation--attendance-flow).*
...
*Implementation details available in `/src/prisma/schema.prisma`. See [Functional Specification - Validation Rules](./functionalSpecification.md#51-input-validation-rules) for field constraints.*
...
#### A
- **API (Application Programming Interface)**: Interface for programmatic access to platform features
- **API Route**: Next.js server-side endpoint for handling HTTP requests
- **Association**: A student organization, club, or group registered on the platform
- **Association Admin**: User with full management permissions for an association [See: [Functional Spec §2.1.4](./functionalSpecification.md#214-association-admin)]
- **Association Member**: User with membership but limited permissions [See: [Functional Spec §2.1.3](./functionalSpecification.md#213-association-member)]
- **Association Profile**: Extended information and settings for an organization [See: [Section 3.2.2](#322-association-profiles)]
- **Authentication**: Process of verifying user identity [See: [Section 3.1](#31-authentication-system)]
- **Authorization**: Process of determining user permissions [See: [Functional Spec §2](./functionalSpecification.md#2-actors-and-roles)]

#### B
- **Badge**: Visual indicator (e.g., verification status, role)
- **bcrypt**: Cryptographic hashing algorithm for passwords
- **Browser Caching**: Storing static resources locally to improve performance
- **Bulk Actions**: Operations performed on multiple items simultaneously

#### C
- **Capacity**: Maximum number of attendees for an event
- **CDN (Content Delivery Network)**: Distributed network for delivering static assets (Vercel Edge Network)
- **CRUD**: Create, Read, Update, Delete operations
- **CSRF (Cross-Site Request Forgery)**: Security vulnerability mitigated by Next.js
- **CSS (Cascading Style Sheets)**: Styling language; using TailwindCSS framework

#### D
- **Dashboard**: Centralized view showing key metrics and actions [See: [Section 3.9](#39-association-management-dashboard), [Functional Spec §3.7](./functionalSpecification.md#37-association-management-dashboard)]
- **Dark Mode**: Alternative color scheme for low-light viewing [See: [Section 4.3](#43-dark-mode)]
- **Database Schema**: Structure of data models and relationships [See: [Section 5.4](#54-database-schema)]
- **Direct Messaging**: One-on-one communication between users [See: [Functional Spec §3.9](./functionalSpecification.md#39-direct-messaging-future)]

#### E
- **Edge Network**: Vercel's globally distributed infrastructure
- **Event**: A scheduled activity organized by an association [See: [Section 3.5](#35-event-management)]
- **Event Registration**: A student's commitment to attend an event [See: [Functional Spec §3.4.3](./functionalSpecification.md#343-event-registration-student)]

#### F
- **Filter**: Method to narrow down search results by criteria
- **Frontend**: Client-side user interface (Next.js + React)

#### G
- **Google OAuth**: Authentication using Google account credentials
- **Gradient**: Color transition effect used throughout UI design
- **Guest User**: Unauthenticated visitor with limited access [See: [Functional Spec §2.1.1](./functionalSpecification.md#211-guest-user-unauthenticated)]

#### H
- **HTTP-only Cookie**: Secure cookie inaccessible to JavaScript
- **HTTPS**: Encrypted HTTP protocol (required)

#### I
- **Image Optimization**: Automatic compression and resizing by Next.js

#### J
- **Join Request**: Student's application to become association member [See: [Functional Spec §3.3.4](./functionalSpecification.md#334-join-association-flow)]
- **JWT (JSON Web Token)**: Token-based session authentication method

#### L
- **Lighthouse Score**: Google's web performance metric (target >90)
- **LinkedIn-style**: Visual design pattern with overlapping card layouts

#### M
- **Member**: User who has active membership in an association
- **Membership**: The relationship between a student and an association [See: [Section 3.4](#34-membership-management)]
- **Membership Status**: State of user-association relationship (PENDING, ACTIVE, INACTIVE, REMOVED, REJECTED)
- **Middleware**: Next.js code that runs before request completion
- **MVP (Minimum Viable Product)**: Core feature set for initial launch [See: [Section 8.1](#81-phase-1-mvp-current)]

#### N
- **Neon**: Serverless PostgreSQL database provider
- **Next.js**: React framework for full-stack web applications (v15.5.6)
- **NextAuth.js**: Authentication library for Next.js (v5)
- **Notification**: System message about platform activity [See: [Section 3.7.2](#372-notifications), [Functional Spec §3.8](./functionalSpecification.md#38-notifications-system)]
...
- **Pending**: Status of membership awaiting approval
- **Performance**: Speed and responsiveness metrics [See: [Section 5.2](#52-performance-requirements)]
- **Persona**: Fictional representative user [See: [Section 2](#2-user-personas)]
- **Post**: An announcement or update from an association [See: [Section 3.6](#36-news-feed--posts)]
- **Prisma**: TypeScript ORM for database operations (v6.19.0)
- **Profile**: User or association information and settings [See: [Section 3.2](#32-user-profiles)]
- **PWA (Progressive Web App)**: Web app with native-app-like features

#### R
- **RBAC (Role-Based Access Control)**: Permission system based on user roles
- **React**: JavaScript library for building user interfaces (v19)
- **Registration Deadline**: Last date to sign up for an event
- **Responsive Design**: UI that adapts to different screen sizes
- **Rich Text Editor**: Interface for formatted text input
- **Role**: Permission level within the system or association [See: [Section 7.3](#73-user-roles--permissions)]

#### S
- **Salt Rounds**: bcrypt hashing iterations (≥10)
- **Schema**: Database structure definition (Prisma)
- **Search**: Finding associations or content by keywords
- **Security**: Protection measures for data and access [See: [Section 5.3](#53-security-requirements)]
- **Session**: Authenticated user's active period on platform
- **Student**: Primary user type seeking to join associations [See: [Functional Spec §2.1.2](./functionalSpecification.md#212-student)]
- **Student Profile**: Extended information for student users [See: [Section 3.2.1](#321-student-profiles)]
- **System Administrator**: User with platform-wide management access [See: [Functional Spec §2.1.5](./functionalSpecification.md#215-system-administrator)]

#### T
- **TailwindCSS**: Utility-first CSS framework (v4.0)
- **Token**: Cryptographic string for authentication (JWT)
- **TypeScript**: Strongly-typed JavaScript superset (v5.x)

#### U
- **UI/UX**: User Interface / User Experience design principles
- **User Flow**: Sequence of steps to complete a task [See: [Section 6](#6-user-flows)]

#### V
- **Validation**: Checking input data meets requirements [See: [Functional Spec §5.1](./functionalSpecification.md#51-input-validation-rules)]
- **Vercel**: Hosting platform with automatic deployments
- **Verification**: Official recognition of an association by the platform [See: [Functional Spec §3.10.2](./functionalSpecification.md#3102-association-verification)]
- **Verification Badge**: Visual indicator of verified status

#### W
- **WCAG (Web Content Accessibility Guidelines)**: Accessibility standards (target: 2.1 AA)
- **WebSocket**: Protocol for real-time bidirectional communication

#### X
- **XSS (Cross-Site Scripting)**: Security vulnerability prevented by React's escaping
